...
-- Tiempo de generación: 13-04-2024 a las 23:54:15
...
--
-- Base de datos: `u600466123_pruebas`
--
...
(13, 'isaac matias', '661ab70ee7680_pruebas (2).sql', 'files/661ab70ee7680_pruebas (2).sql', '661ab70ee767d_descarga.png', 'image/661ab70ee767d_descarga.png', '6', 'es una base de datos de prueba', 1, 0),
(14, 'Miselanea', '661ab901b81a4_actividad 1 programacion.docx', 'files/661ab901b81a4_actividad 1 programacion.docx', '661ab901b81a2_logincod.png', 'image/661ab901b81a2_logincod.png', '2', 'jolse ', 1, 0);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`u600466123_luispc`@`127.0.0.1` SQL SECURITY DEFINER VIEW `archivos_usuarios`  AS SELECT `usuarios`.`id` AS `id`, `usuarios`.`username` AS `username`, `archivo`.`id` AS `archivo`, `archivo`.`nombre` AS `nombre`, `archivo`.`rutai` AS `rutai` FROM (`usuarios` join `archivo` on(`usuarios`.`id` = `archivo`.`usuario`)) ;
